...
(async()=>{
  ...
  c._details={type:'tv',selectedEpisode:{episode_number:1},selectedSeason:1,episodeSources:[
  ...
  assert.equal(c.events.at(-1)[2].activity,'netflix://title/12345',
    'v0.4.82 native Netflix route works for selected series links');
  c.events=[];
  c._details={type:'tv',selectedEpisode:{episode_number:1},selectedSeason:1,episodeSources:[
  ...
  assert.equal((await c._watchmodeSourcesForCurrentTitle())[0].source,'watchhub',
    'WatchHub can still fill a gap when the old lookup has no link');
  ...
  console.log('PASS v0.4.82 Netflix native episode route, Prime original URL, old source priority, optional WatchHub fallback, and Roku UI guard');
